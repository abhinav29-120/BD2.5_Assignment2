...
let stocks = [
    ...
    { id: 4, name: "tata consultancy services", price: 3200, growth: 2.9, industry: "finance", exchange: "bse", price: 1900, },
    ...
    { id: 8, name: "cipla", growth: 2.6, price: 2100, exchange: "bse", industry: "pharma", },
...
function sortStocksByPrice(ele1,ele2,PassOrder){
    // let val=PassOrder(vl);
...
app.get("/stocks/sort/pricing",(req,res)=>{
    ...
    let arrCp=stocks.slice();
    let sortedStocks=arrCp.sort((el1,el2)=>sortStocksByPrice(el1,el2,order));
    res.json({ stocks: sortedStocks });
});

//Endpoint 2: Get the stocks sorted based on their Growth.
function SortStocksByGrowth(ele1,ele2,PassOrder){
    if(PassOrder==="low-to-high"){
...
app.get("/stocks/sort/growth",(req,res)=>{
    let order=req.query.growth;
    let arrCp=stocks.slice();
    let sortedStocks=arrCp.sort((a,b)=>SortStocksByGrowth(a,b,order));
    res.json({stocks: sortedStocks});
});

// Endpoint 3: Filter the stocks based on the 2 Stock Exchange (NSE. and BSE)
function filterByExchange(Objele,exVal){
    return (Objele.exchange).toLowerCase()===exVal.toLowerCase();
}
app.get("/stocks/filter/exchange",(req,res)=>{
    let exVal=req.query.exchange;
    let arrCp=stocks.slice();
    let sortedStocks=arrCp.filter(arrObj=>filterByExchange(arrObj,exVal));
    res.json({stocks: sortedStocks});
});

//Endpoint 4: Filter the stocks based on the Industrial Sector.
function filterByIndustry(Objele,inVal){
    return (Objele.industry).toLowerCase()===inVal.toLowerCase();
};
app.get("/stocks/filter/industry",(req,res)=>{
    let inVal=req.query.industry;
    let arrCp=stocks.slice();
    let sortedStocks=arrCp.filter(arrObj=>filterByIndustry(arrObj,inVal));
    res.json({stocks:sortedStocks});
});
